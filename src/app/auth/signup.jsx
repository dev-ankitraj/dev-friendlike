...
export default function SignUpPage() {
    ...
    const [credentials, setCredentials] = useState({ userId: '', email: '', password: '', userName: '' });
    const [confirmPassword, setConfirmPassword] = useState('');
    const [message, setMessage] = useState({ error: false, text: '' });

    // Function to handle email and password input submission
    const handleEmailPassWord = async () => {
        if (isValidEmail(credentials.email) && credentials.password && (credentials.password === confirmPassword)) {
            setFormStep(1);
            try {
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${Friend_Link_Server.Server()}/user/create`, credentials);

            if (res.status == 200) {
                SaveAuthenticatedUserStatus(res.data);
                const resEmail = await axios.post(`${Friend_Link_Server.Server()}/email/welcome`, { email: credentials.email, userName: credentials.userName });
            }
        } catch (err) {
                    ...
                    <form className='flex flex-col text-center md:w-96 w-full gap-2 p-2' onSubmit={handleFormSubmit}>
                        ...
                        {formStep !== 0 ? "" :
                            <>
                                ...
                                <button onClick={handleEmailPassWord} type='button'
                        ...
                        {formStep !== 2 ? "" :
                            <SignUpStep2 credentials={credentials} setCredentials={setCredentials} handleAppLog={handleAppLog} />
                        }
...
const SignUpStep2 = ({ credentials, setCredentials, handleAppLog }) => {
    // Regular expression to validate userId format
    const regex = /^[a-z0-9_-]*$/;

    // State to track if the userId is unique
    ...
    useEffect(() => {
        const checkUserId = async () => {
            // Validate userId against regex
            if (regex.test(credentials.userId)) {
                // Check if userId length is more than 5 characters
                if (credentials.userId.length > 5) {
                    try {
                        // Fetch user profile data from the server based on credentials.userId
                        const res = await axios.get(`${Friend_Link_Server.Server()}/user/profile/${credentials.userId}`);

                        if (res.status == 200) {
                            // User id already taken
                            setMessage({ ...message, error: true, text: 'User ID not availability.' });
                            setIsUniqueUserId(false);
                        }
                    } catch (err) {
                        setIsUniqueUserId(true);
                        setMessage({ ...message, error: false, text: 'Available' });
                    }
                } else {
                    // Handle case where userId is too short
                    setIsUniqueUserId(false);
                    setMessage({ ...message, error: true, text: 'User ID must be more than 5 characters' });
                }
            } else {
                // Handle case where userId contains invalid characters
                setIsUniqueUserId(false);
                setMessage({ ...message, error: true, text: 'Invalid character used' });
            }
        };

        // Call the checkUserId function to validate userId
        checkUserId();
    }, [credentials.userId]);

    // Cleanup function to reset message error state
    useEffect(() => {
        return () => setMessage({ error: false, text: '' });
    }, []);
    ...
    return (
        <div className='flex flex-col w-full gap-2'>
            ...
            <div className='flex flex-col w-full gap-2 text-start relative'>
                <label htmlFor="userId">UserId: </label>
                <div className='flex gap-2 h-10 items-center dark:bg-dark-3 bg-light-3 px-2 rounded-lg'>
                    <input type='text' placeholder='unique id' id='userId' className='px-0'
                    ...
                    {isUniqueUserId ?
                        <>
                            ...
                            <p className='text-xs text-green-400 absolute right-0 -bottom-4'>{message.text}</p>
                        </>
                        :
                        credentials.userId.length < 6 ? '' :
                            <IoCloseCircleOutline className='text-red-400' />}

                    {message.error ?
                        <p className='text-xs text-red-400 absolute right-0 -bottom-4'>{message.text}</p>
                        : ''}
